...
import { calculateFifo, PortfolioSummary } from "../utils/fifo";
import { formatEuro, formatPercent } from "../utils/format";
import { MoneyValue, ReturnValue } from "./MoneyValue";
import { Skeleton } from "@/components/ui/skeleton";
import { TrendingUp, TrendingDown, Wallet, BarChart3, Award } from "lucide-react";
...
function SummaryCard({ label, value, icon, className, delay = 0 }: SummaryCardProps) {
  return (
    <div
      className={cn(
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {["inv", "cur", "real", "unreal", "total"].map((key) => (
          <div key={key} className="bg-card border border-border rounded-lg p-5 flex flex-col gap-3">
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      ...
      <SummaryCard
        ...
        value={<MoneyValue amount={summary.totalCurrentValue} className="text-xl font-semibold" />}
        delay={50}
      />
      <SummaryCard
        label="Gerealiseerd"
